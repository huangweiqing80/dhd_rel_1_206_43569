#!/bin/bash
#
# Create the bwlver.h file from version.h.in
# Then, bwlver.h is renanmed to bwl_version.h by the Makefile in this dir.
# bwl.h includes bwl_version.h which contains the version string.
#
# $Id: bwlver.sh,v 1.2 2010-12-02 18:47:31 $

# Check for the in file, if not there we're probably in the wrong directory
if [ ! -f bwlver.h.in ]; then
	echo No bwlver.h.in found
	exit 1
fi
...
if [ -f bwlver.h ]; then
	...
	build=`grep MOD_BUILD_NUMBER bwlver.h | sed -e "s,.*BUILD_NUMBER[ 	]*,,"`
	build=`expr ${build} + 1`
	echo build=${build}
	sed -e "s,.*_BUILD_NUMBER.*,#define MOD_BUILD_NUMBER	${build}," \
		< bwlver.h > bwlver.h.new
	mv bwlver.h bwlver.h.prev
	mv bwlver.h.new bwlver.h
else
