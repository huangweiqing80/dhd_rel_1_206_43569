 ...
 *
 * $Id: wps_ap.c 470764 2014-04-16 08:40:23Z $
 */
...
wpsap_process_msg(wpsap_wksp_t *wps_wksp, char *buf, uint32 len, uint32 msgtype)
{
	...
	case TRANSPORT_TYPE_EAP:
		WPS_HexDumpAscii(TUDUMP_MSG, "<== From EAPOL", (unsigned char *)buf, len);
	...
	case TRANSPORT_TYPE_UPNP_DEV:
	{
		...
		snprintf(title, sizeof(title), "<== From UPnP - %s", wps_upnp_type_name(type));
		WPS_HexDumpAscii(TUDUMP_MSG, title, (unsigned char *)buf, len);
...
wpsap_readConfigure(wpsap_wksp_t *wps_wksp, DevInfo *ap_devinfo)
{
	...
	wps_strncpy(ap_devinfo->deviceName, value, sizeof(ap_devinfo->deviceName));
	...
	/* Zero length mandatory string attributes such as serial number,
	 * which are not used in the security calculations.
	 */
	if (strcmp(wps_safe_get_conf("wps_zlength"), "1") == 0)
		ap_devinfo->b_zlength = true;
	else
		ap_devinfo->b_zlength = false;
#endif /* WFA_WPS_20_TESTBED */
...
wpsap_wksp_init(wpsap_wksp_t *ap_wksp)
{
	...
	if (ap_wksp->sc_mode == SCMODE_AP_ENROLLEE) {
		...
		if (strcmp(nfc_ap_pin, "NFC_PW") == 0 || strcmp(nfc_ap_pin, "NFC_CHO") == 0) {
			dev_pin = wps_nfc_dev_pw_str();
			devicepwid = wps_nfc_pw_id();
			if (strcmp(nfc_ap_pin, "NFC_CHO") == 0) {
				/* CHO need to verify the peer public key hash */
				pub_key_hash = wps_nfc_pub_key_hash();
			}
		}
#endif
		ret = wpsap_start_enrollment_devpwid(ap_wksp->mc, dev_pin, pub_key_hash,
			devicepwid);
	}
	else {
	...
	if (ap_eap_sm_init(ap_wksp->mc, (char *)ap_wksp->mac_sta, wps_eapol_parse_msg,
	...
	if (opcode == WPS_SUCCESS) {
		...
		if (err_code == WPS_ERR_PBC_OVERLAP) {
...
#ifdef WPS_NFC_DEVICE
		} else if (err_code == RPROT_ERR_DEV_PW_ID_MISMATCH ||
			err_code == RPROT_ERR_PUB_KEY_HASH_MISMATCH) {
			wps_setProcessStates(err_code == RPROT_ERR_DEV_PW_ID_MISMATCH ?
				WPS_NFC_HO_DPI_MISMATCH : WPS_NFC_HO_PKH_MISMATCH);
			if (sc_mode == SCMODE_AP_REGISTRAR)
				wps_wksp->return_code = WPS_RESULT_REGISTRATION_CHOFAIL;
			else if (sc_mode == SCMODE_AP_ENROLLEE)
				wps_wksp->return_code = WPS_RESULT_ENROLLMENT_CHOFAIL;
#endif
		} else {
			...
			if (err_code != WPS_M2D_NACK_CONT) {
				...
				/* Set process state to init for Win7 enrollee, because we don't
				 * know if Win7 want to do WPS PBC or use Security Key latter.
				 * I think failed at M2D NACK mean notthing and it only happens in
				 * Win7 enrollee, so we just turn the WPS led off to prevent keep
				 * blinking problem.
				 */
