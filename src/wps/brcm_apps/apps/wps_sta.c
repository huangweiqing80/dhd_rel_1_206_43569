...
static bool b_wsec_auto = false;
static bool b_bssid_auto = false;
...
static bool wpssta_find_selreg_ap(char *ssid, uint8 *bssid, uint8 *wsec);
...
wpssta_do_join(uint8 again)
{
	...
	join_network_with_bssid(ssid, wsec, (char *)bssid);
...
found:
	...
	if (pbc ==  WPS_UI_PBC_HW) {
		...
		if (atoi(wps_ui_get_env("wps_method")) == WPS_UI_METHOD_PBC) {
...
#ifdef WPS_NFC_DEVICE
			char *sta_pin = wps_ui_get_env("wps_sta_pin");
#endif
			strcat(op, "pin");

#ifdef WPS_NFC_DEVICE
			if (strcmp(sta_pin, "NFC_PW") == 0 || strcmp(sta_pin, "NFC_CHO") == 0) {
				env_pin = wps_nfc_dev_pw_str();
				oob_devpwid = wps_nfc_pw_id();
				pre_privkey = wps_nfc_priv_key();
				if (strcmp(sta_pin, "NFC_CHO") == 0) {
			...
			if (strcmp(env_bssid, "00:00:00:00:00:00") == 0) {
				b_bssid_auto = true;
			}
			else if (strlen(env_bssid)) {
				/*
			...
			if (sta_wksp->sc_mode == SCMODE_STA_REGISTRAR) {
				...
				if (strcmp(env_pin, "NFC_PW") == 0 ||
				    strcmp(env_pin, "NFC_CHO") == 0) {
					env_pin = wps_nfc_dev_pw_str();
					oob_devpwid = wps_nfc_pw_id();
					pub_key_hash = wps_nfc_pub_key_hash();
					if (strcmp(env_pin, "NFC_CHO") == 0) {
						pre_privkey = wps_nfc_priv_key();
						b_choing = true;
					}
				}
	 ...
	 */
	if (sta_wksp->sc_mode == SCMODE_STA_REGISTRAR) {
		if (wpssta_reg_config_init(sta_wksp, ifname, (char *)bssid, oob) != WPS_SUCCESS) {
...
#ifdef WPS_NFC_DEVICE
		/* We don't want STA do WPS re-open windows when it receive
		 * EAP-Failure before M2.  So here STA need to wait the AP selreg
		 * set and start join.  This can prevent the STA receive EAP-Failure
		 * on the proxy mode case.
		 */
		if (b_choing) {
			wpsenr_osl_proc_states(WPS_FIND_SEL_AP);
...
wpssta_wksp_start(wpssta_wksp_t *sta_wksp)
{
	...
	if (pbc !=  WPS_UI_PBC_HW) {
		val = wps_ui_get_env("wps_enr_bssid");
		if (val)
			user_bssid = true;
	}
	...
	if (wps_get_bssid((char *)assoc_bssid)) {
	...
	if (wps_osl_init((char *)bssid) != WPS_SUCCESS) {
	...
	if (sta_wksp->sc_mode == SCMODE_STA_REGISTRAR) {
		...
		if (wpssta_start_enrollment_devpwid(pin, pub_key_hash, oob_devpwid, start_time)
			!= WPS_SUCCESS) {
	...
	if (retVal == WPS_SUCCESS) {
		...
		if (retVal == WPS_SEND_MSG_CONT ||
			...
			if (last_recv_msg > WPS_ID_MESSAGE_M2D) {
				...
				uint32 err_code = wpssta_get_err_code();

				wpsenr_osl_proc_states(err_code == RPROT_ERR_DEV_PW_ID_MISMATCH ?
					WPS_NFC_HO_DPI_MISMATCH :
					err_code == RPROT_ERR_PUB_KEY_HASH_MISMATCH ?
					WPS_NFC_HO_PKH_MISMATCH : WPS_MSG_ERR);
				b_state_set = TRUE;
				retVal = REG_FAILURE;
			}
#endif
...
{
	int state, find_pbc = PBC_NOT_FOUND;
	int last_sent_msg;
	uint8 msg_type;
	char *sendBuf;
	uint32 sendBufLen;
	...
	case WPS_ASSOC_STATE_FINDINGAP:
#ifdef WPS_NFC_DEVICE
		/* find selreg specific ap */
		if (b_choing) {
			uint8 *wsec_auto = NULL, *bssid_auto = NULL;

			if (b_wsec_auto)
				wsec_auto = &wsec;
			if (b_bssid_auto)
				bssid_auto = bssid;
			find_selreg = wpssta_find_selreg_ap(ssid, bssid_auto, wsec_auto);
		}
		else
#endif
		{
			/* find pbc ap */
			find_pbc = wps_pb_find_pbc_ap((char *)bssid, ssid, &wsec);
	...
	case WPS_ASSOC_STATE_ASSOCIATING:
		...
		else if (wps_get_bssid((char *)assoc_bssid) == 0) {
			/* associated with AP */
	...
	if (retVal == WPS_SEND_RET_MSG_CONT) {
		sendBufLen = wps_get_retrans_msg_to_send(&sendBuf, now, (char *)&msg_type);
		if (sendBuf) {
			state = wps_get_eap_state();
...
wpssta_do_protocol_again(wpssta_wksp_t *sta_wksp)
{
	...
	if (sta_wksp->sc_mode == SCMODE_STA_REGISTRAR) {
		...
		ret = wpssta_start_enrollment_devpwid(pin, pub_key_hash, oob_devpwid, current_time);
	}
...
/* check is the specific AP SSID (bssid is optional) has select reg bit set */
static bool
wpssta_find_selreg_ap(char *ssid, uint8 *bssid, uint8 *wsec)
{
	int wps_apcount = 0;
