 ...
 *
 * $Id: wps_eap.c 470764 2014-04-16 08:40:23Z $
 */
...
wps_eap_process_msg(char *buf, int buflen)
{
	...
	if (ntohs(eth->ether_type) == ETHER_TYPE_BRCM) {
		...
		if (event_type == WLC_E_PROBREQ_MSG) {
			...
			wps_wfi_process_sta_ind(ie, totlen, addr, (uint8*)event->ifname, TRUE);
			return ret;
		}
		else if ((event_type == WLC_E_DISASSOC_IND) ||	(event_type == WLC_E_DEAUTH_IND)) {
			wps_wfi_process_sta_ind(ie, totlen, addr, (uint8*)event->ifname, FALSE);
			return ret;
		}
#endif /* BCMWPSAP && __CONFIG_WFI__ */
#endif /* BCMWFI */
...
#ifdef WPS_NFC_DEVICE
			if (strcmp(wps_ui_get_env("wps_sta_pin"), "NFC_CHO") == 0)
	...
	ret = wpsap_open_session(wps_app, mode, eapol->eth.ether_dhost, eapol->eth.ether_shost,
		ifname, NULL, pre_privkey, (uint8 *)authorizedMacs, authorizedMacs_len, 0, 0);
#endif /* BCMWPSAP */
	return ret;
}
