 ...
 *
 * $Id: wps_ie.c 470764 2014-04-16 08:40:23Z $
 */
...
wps_set_prb_rsp_IE(int ess_id, char *ifname, CTlvSsrIE *ssrmsg)
{
	...
	CTlvVendorExt vendorExt;
#ifdef WFA_WPS_20_TESTBED
	bool b_zlength = atoi(wps_safe_get_conf("wps_zlength")) ? TRUE : FALSE;
#endif /* WFA_WPS_20_TESTBED */
	...
	tlv_serialize(WPS_ID_MANUFACTURER, bufObj, pc_data, STR_ATTR_LEN(pc_data, SIZE_64_BYTES));

	/* Model Name */
	pc_data = wps_safe_get_conf("wps_modelname");
	tlv_serialize(WPS_ID_MODEL_NAME, bufObj, pc_data, STR_ATTR_LEN(pc_data, SIZE_32_BYTES));

	/* Model Number */
	pc_data = wps_safe_get_conf("wps_modelnum");
	tlv_serialize(WPS_ID_MODEL_NUMBER, bufObj, pc_data, STR_ATTR_LEN(pc_data, SIZE_32_BYTES));

	/* Serial Number */
	pc_data = wps_safe_get_conf("boardnum");
	tlv_serialize(WPS_ID_SERIAL_NUM, bufObj, pc_data, STR_ATTR_LEN(pc_data, SIZE_32_BYTES));
	...
	tlv_serialize(WPS_ID_DEVICE_NAME, bufObj, pc_data, STR_ATTR_LEN(pc_data, SIZE_32_BYTES));
	...
	if (pc_data)
		config_method = (uint16)(strtoul(pc_data, NULL, 16));
	...
	if (b_wps_version2) {
		...
		/* Registrar Configuration Method */
		/* The Configuration Methods supported by the internal Registrar of the AP.
		 * If the AP is an NFC Device then the Registrar Configuration Methods subelement
		 * MUST be included, otherwise it MAY be included.
		 */
		subtlv_serialize(WPS_WFA_SUBID_REG_CFG_METHODS, vendorExt_bufObj,
			&config_method, WPS_WFA_SUBID_REG_CFG_METHODS_S);
