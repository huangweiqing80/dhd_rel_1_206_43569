 ...
 *
 * $Id: wps_libupnp.c 470127 2014-04-14 04:14:51Z $
 */
	...
	char *databuf, int datalen, int init, char event_type)
{
	...
	snprintf(title, sizeof(title), "==> To UPnP - %s", wps_upnp_type_name(UPNP_WPS_TYPE_WE));
	if (event_type == WPS_WLAN_EVENT_TYPE_PROBE_REQ_FRAME) {
		if (!WPS_HexDumpAscii(TUDUMP_PROBE, title, (unsigned char *)databuf, datalen)) {
			TUTRACE((TUTRACE_INFO, "Forward PROBE REQUEST "
		...
		WPS_HexDumpAscii(TUDUMP_MSG, title, (unsigned char *)databuf, datalen);
	}
	...
	wps_upnp_base64_encode((unsigned char *)buf, datalen+bytes, (unsigned char *)base64buf);
...
wps_upnp_update_init_wlan_event(int if_instance, char *mac, int init)
{
	...
	wps_upnp_update_wlan_event(if_instance, (unsigned char *)mac,
		(char *)bufObj->pBase, bufObj->m_dataLength, init,
...
wps_upnp_get_ssr_ifname(int ess_id, char *client_ip, char *wps_ifname)
{
	...
	foreach(ifname, wlnames, next) {
		...
		count = wps_wl_get_maclist(ifname, (char *)mac_list, (sizeof(mac_list)/6));
		if (count == -1) {
...
wps_upnp_create_device_info(upnp_attached_if *upnp_if)
{
	...
	if (reg_proto_BN_bn2bin(devInfo.DHSecret->priv_key,
		(unsigned char *)upnp_if->private_key) != SIZE_PUB_KEY) {
...
wps_upnp_send_msg(int if_instance, char *buf, int len, int type)
{
	...
	snprintf(title, sizeof(title), "==> To UPnP - %s", wps_upnp_type_name(type));
...
wps_libupnp_ProcessMsg(char *ifname, char *upnpmsg, int upnpmsg_len)
{
	...
	WPS_HexDumpAscii(TUDUMP_MSG, title, (unsigned char *)data, data_len);
