 ...
 *
 * $Id: wps_wl.c 470127 2014-04-14 04:14:51Z $
 */
...
wps_wl_del_vndr_ie(char *wl_if, char *bufaddr, int buflen, uint32 frametype)
{
	...
	snprintf(title, sizeof(title), "wps_wl_del_vndr_ie (%s)", _pktflag_name(frametype));
	WPS_HexDumpAscii(TUDUMP_IE, title, frag, frag_len);
	...
	unsigned int pktflag)
{
	...
	snprintf(title, sizeof(title), "wps_wl_set_vndr_ie (%s)", _pktflag_name(pktflag));
	WPS_HexDumpAscii(TUDUMP_IE, title, frag, frag_len);
...
wps_wl_bss_config(char *ifname, int enabled)
{
	...
	snprintf(tmp, sizeof(tmp), "%s_ure_mode", ifname);
	...
	sprintf(tmp, "%s_bss_enabled", ifname);
	if (enabled)
		wps_set_conf(tmp, "1");
	else
		wps_set_conf(tmp, "0");
...
int
wps_wl_channel(char *ifname)
{
	uint32 chanspec;

	if (wl_iovar_getint(ifname, "chanspec", (int*)(void *)&chanspec))
		return 0;

	return wf_chspec_ctlchan(chanspec);
}
