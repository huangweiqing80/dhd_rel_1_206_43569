 ...
 *
 * $Id: wpscli_sta.c 475451 2014-05-05 20:44:21Z $
 *
...
static brcm_wpscli_status wpscli_create_select_wps_ap_list(int *nAP);
...
brcm_wpscli_status wpscli_sta_set_prod_info(brcm_wpscli_product_info *prodInfo)
{
	/* Sanity check */
	if (!prodInfo)
	{
	...
	{
		memcpy(sta_devinfo.manufacturer, prodInfo->manufacturer, SIZE_64_BYTES);
		sta_devinfo.manufacturer[sizeof(sta_devinfo.manufacturer)-1] = 0;
	}
	if ((prodInfo->modelName[0] != 0) &&
	    (strlen(prodInfo->modelName) > 0) &&
	    (strlen(prodInfo->modelName) <= SIZE_32_BYTES))
	{
		memcpy(sta_devinfo.modelName, prodInfo->modelName, SIZE_32_BYTES);
		sta_devinfo.modelName[sizeof(sta_devinfo.modelName)-1] = 0;
	}
	if ((prodInfo->modelNumber[0] != 0) &&
	    (strlen(prodInfo->modelNumber) > 0) &&
	    (strlen(prodInfo->modelNumber) <= SIZE_32_BYTES))
	{
		memcpy(sta_devinfo.modelNumber, prodInfo->modelNumber, SIZE_32_BYTES);
		sta_devinfo.modelNumber[sizeof(sta_devinfo.modelNumber)-1] = 0;
	}
	if ((prodInfo->serialNumber[0] != 0) &&
	    (strlen(prodInfo->serialNumber) > 0) &&
	    (strlen(prodInfo->serialNumber) <= SIZE_32_BYTES))
	{
		memcpy(sta_devinfo.serialNumber, prodInfo->serialNumber, SIZE_32_BYTES);
		sta_devinfo.serialNumber[sizeof(sta_devinfo.serialNumber)-1] = 0;
	}
	if (prodInfo->osVersion != 0)
	{
		/* WSC spec: hi bit is reserved and must be set */
		sta_devinfo.osVersion = prodInfo->osVersion | 0x80000000;
	}
...
// without conducting WPS negotiation after scanning, so we need provide a separate wps
// scan API for that
brcm_wpscli_status brcm_wpscli_sta_search_select_wps_ap(uint32 *wps_ap_total)
{
	brcm_wpscli_status status = WPS_STATUS_SUCCESS;
	...
	if(status == WPS_STATUS_SUCCESS)
		// Convert and generate wps-capable ap list in wps_ap_list_info_t format
		status = wpscli_create_select_wps_ap_list((int *)&g_WpsApTotal);
...
brcm_wpscli_status brcm_wpscli_sta_get_wps_ap_list(brcm_wpscli_ap_list *wps_ap_list, uint32 buf_size, uint32 *wps_ap_total)
{
	...
	*wps_ap_total = g_WpsApTotal;
	...
	for (i=0; i<g_WpsApTotal; i++)
	{
		...
		// get channel and channel band info from chanspec
		wpscli_ap_entry->chanspec = bi->chanspec;
    ...
    for (i = 0; i < list->count; i++) {
        ...
        if (bi->ie_length) {
			if(wps_ap_count < WPS_MAX_AP_SCAN_LIST_LEN) {
				...
				ap_list[wps_ap_count].chanspec = bi->chanspec;  /* copy over the chanspec info */				
				...
				ap_list[wps_ap_count].band = (bi->ctl_ch <= CH_MAX_2G_CHANNEL) ? WPS_RFBAND_24GHZ : WPS_RFBAND_50GHZ;  // Use channel number to decide band
...
brcm_wpscli_status wpscli_create_select_wps_ap_list(int *nAP)
{
	brcm_wpscli_status status = WPS_STATUS_SUCCESS;
	wps_ap_list_info_t *wpsaplist = ap_list;  // Point to the same AP list memory allocatoin
	...
	TUTRACE((TUTRACE_INFO, "wpscli_create_select_wps_ap_list: Entered.\n"));
	...
	{
		if(*nAP > 0) {
			TUTRACE((TUTRACE_INFO, "wpscli_create_select_wps_ap_list: wpscli_convert_ap_list_format returns %d AP\n", *nAP));

			// After this call, the first wps_ap_total elements in the ap_list are wps ones
			*nAP = wps_get_select_aplist(wpsaplist, wpsaplist);
			TUTRACE((TUTRACE_INFO, "wpscli_create_select_wps_ap_list: wps_get_select_aplist returns %d AP\n", *nAP));
	...
	TUTRACE((TUTRACE_INFO, "wpscli_create_select_wps_ap_list: Exiting.\n"));
	return status;
}
...
int wpscli_on_evt_eap_pkt_received(const char *data, uint32 data_len, uint8 *last_sent_msg)
{
	...
	if (retVal == WPS_SEND_MSG_CONT ||
		...
		if (retVal == WPS_ERR_ENROLLMENT_PINFAIL) {
			...
			"wpscli_on_evt_eap_pkt_received: goto exit. Wrong PIN. retVal=%d.\n",
			retVal));
			goto exit;
		}
