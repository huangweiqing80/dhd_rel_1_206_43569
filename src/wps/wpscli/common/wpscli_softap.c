 ...
 *
 * $Id: wpscli_softap.c 475451 2014-05-05 20:44:21Z $
 *
...
#ifdef WIDI
#define WIDI_OUI		0x00173520	// defined in wfd_ie.h
#endif
...
static BOOL g_bWpsSessionOpen = FALSE;
...
brcm_wpscli_status wpscli_softap_construct_def_devinfo()
{
	...
	softap_devinfo.secDeviceCategory = 0;
	softap_devinfo.secDeviceSubCategory = 0;
#ifdef WIDI
	softap_devinfo.secDeviceOui = WIDI_OUI;
#else
	softap_devinfo.secDeviceOui = 0x0050F204;
#endif
#endif	
...
brcm_wpscli_status wpscli_softap_set_prod_info(brcm_wpscli_product_info *prodInfo)
{
	/* Sanity check */
	if (!prodInfo)
	{
	...
	if ((prodInfo->manufacturer[0] != 0) &&
	    (strlen(prodInfo->manufacturer) > 0) &&
	    (strlen(prodInfo->manufacturer) <= SIZE_64_BYTES))
	{
		memcpy(softap_devinfo.manufacturer, prodInfo->manufacturer, SIZE_64_BYTES);
		softap_devinfo.manufacturer[sizeof(softap_devinfo.manufacturer)-1] = 0;
	}
	if ((prodInfo->modelName[0] != 0) &&
	    (strlen(prodInfo->modelName) > 0) &&
	    (strlen(prodInfo->modelName) <= SIZE_32_BYTES))
	{
		memcpy(softap_devinfo.modelName, prodInfo->modelName, SIZE_32_BYTES);
		softap_devinfo.modelName[sizeof(softap_devinfo.modelName)-1] = 0;
	}
	if ((prodInfo->modelNumber[0] != 0) &&
	    (strlen(prodInfo->modelNumber) > 0) &&
	    (strlen(prodInfo->modelNumber) <= SIZE_32_BYTES))
	{
		memcpy(softap_devinfo.modelNumber, prodInfo->modelNumber, SIZE_32_BYTES);
		softap_devinfo.modelNumber[sizeof(softap_devinfo.modelNumber)-1] = 0;
	}
	if ((prodInfo->serialNumber[0] != 0) &&
	    (strlen(prodInfo->serialNumber) > 0) &&
	    (strlen(prodInfo->serialNumber) <= SIZE_32_BYTES))
	{
		memcpy(softap_devinfo.serialNumber, prodInfo->serialNumber, SIZE_32_BYTES);
		softap_devinfo.serialNumber[sizeof(softap_devinfo.serialNumber)-1] = 0;
	}
	if (prodInfo->osVersion != 0)
	{
		/* WSC spec: hi bit is reserved and must be set */
		softap_devinfo.osVersion = prodInfo->osVersion | 0x80000000;
	}
  ...
  if(status == WPS_STATUS_SUCCESS)  {
      ...
      if(eapol->type == EAPOL_START)
	{
	  ...
	  return WPS_STATUS_PROTOCOL_CONTINUE;
	    ...
	    if(status != WPS_STATUS_SUCCESS) {
		    TUTRACE((TUTRACE_ERR, "brcm_wpscli_softap_process_eapwps: Failed to initialize wps session.\n"));
		    goto FAIL;
	    }
						 ...
						 uint32 authorizedMacs_len)
{
	...
	if (e_mode == SCMODE_AP_ENROLLEE) {
		 ...
		 *
		 * This is the same WAR in WPS we used on physical router side 
		 */
		WpsSleep(1);
	...
	if(uRet != WPS_SUCCESS) {
		TUTRACE((TUTRACE_ERR, "wpscli_softap_init_wps: Failed to start wpsap. Return error %d, e_mode %d.\n", uRet, e_mode));
		return WPS_STATUS_PROTOCOL_INIT_FAIL;
	}
...
#else
		uint16 passwdId, char *deviceName, uint16 reqCategoryId, uint16 reqSubCategoryId,
		uint32 secDevOui, uint8 secDevType, uint8 secDevSubCat)
#endif
{
	BOOL bRet = FALSE;
	int buflen = maxLength;
...
#ifdef SECONDARY_DEVICE_TYPE
		softap_devinfo.secDeviceCategory	= secDevType;
		softap_devinfo.secDeviceSubCategory = secDevSubCat;
		softap_devinfo.secDeviceOui = secDevOui;
	
#ifdef WIDI
		if (softap_devinfo.secDeviceOui == WIDI_OUI)
			softap_devinfo.secDeviceSubCategory |= 0x0100;
#endif
#endif
...
int wpscli_softap_encode_probe_response_wps_ie(char *buf, int maxLength,
	uint16 cfgMeths, uint16 primDevCategory, uint16 primDevSubCategory,
#ifdef SECONDARY_DEVICE_TYPE
		uint8 secDevType, uint8 secDevSubCat,
#endif
	uint16 passwdId, char *deviceName, uint16 selRegistrar)
{
	BOOL bRet = FALSE;
	int buflen = maxLength;


	TUTRACE((TUTRACE_INFO, "wpscli_softap_encode_probe_request_wps_ie. selRegistrar=%d: Entering.\n", selRegistrar));
	/* Set softap_devinfo */
	softap_devinfo.configMethods = cfgMeths;
	softap_devinfo.primDeviceCategory = primDevCategory;
	softap_devinfo.primDeviceSubCategory = primDevSubCategory;
	...
	softap_devinfo.secDeviceSubCategory = secDevSubCat;
#endif	
	softap_devinfo.selRegistrar = selRegistrar;
	softap_devinfo.devPwdId = passwdId;
	strcpy(softap_devinfo.deviceName, deviceName);
...
static brcm_wpscli_status wpscli_softap_add_wps_ie(void)
{
	...
	/* Set softap_devinfo */
#ifdef SECONDARY_DEVICE_TYPE
		/* defrisco: use primary device type as secondary device so we continue to be seen as a display device */
		softap_devinfo.secDeviceCategory = softap_devinfo.primDeviceCategory;
		softap_devinfo.secDeviceSubCategory = softap_devinfo.primDeviceSubCategory;
		memcpy(&softap_devinfo.secDeviceOui, &softap_devinfo.primDeviceOui, sizeof(softap_devinfo.secDeviceOui));
#endif
	...
	if(b_wps_version2)
		...
		return (bRet_BE & bRet_PR) ? WPS_STATUS_SUCCESS : WPS_STATUS_SET_WPS_IE_FAIL;
}
