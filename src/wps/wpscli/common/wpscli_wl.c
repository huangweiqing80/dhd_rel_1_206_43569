 ...
 *
 * $Id: wpscli_wl.c 475022 2014-05-02 23:21:49Z $
 *
...
BOOL wpscli_iovar_setbuf(const char *iovar, void *param, uint paramlen, void *buf, int buf_len)
{
	BOOL bRet = FALSE;
	uint iolen;
	brcm_wpscli_status status;

	iolen = wpscli_iovar_mkbuf(iovar, param, paramlen, buf, buf_len);
	if (iolen == 0)
	{
		TUTRACE((TUTRACE_ERR, "wpscli_iovar_setbuf(%s, paramlen=%d): wpscli_iovar_mkbuf() failed\n"));
		return bRet;
	}

	status = wpscli_wlh_ioctl_set(WLC_SET_VAR, buf, iolen);
	if(status == WPS_STATUS_SUCCESS)
		bRet = TRUE;
		...
		if (b_wps_version2) {
			...
			if (devinfo->authorizedMacs_len) {
				...
				memcpy(proberesp->authorizedMacs.macs, devinfo->authorizedMacs,
					...
					proberesp->secDeviceCategory	= devinfo->secDeviceCategory;
					proberesp->secDeviceSubCategory = devinfo->secDeviceSubCategory;
					proberesp->secDeviceOui 		= devinfo->secDeviceOui;
#endif
