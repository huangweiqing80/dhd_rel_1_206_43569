 ...
 *
 * $Id: ap_api.c 470764 2014-04-16 08:40:23Z $
 */

#if !defined(__linux__) && !defined(_MACOSX_)&& !defined(TARGETOS_symbian)
#include <stdio.h>
#include <windows.h>
#endif
...
wpsap_start(WPSAPI_T *g_mc, char *pin)
{
	...
	case SCMODE_AP_ENROLLEE:
		...
		/* Actually we don't need mp_tlvEsM7Ap in the case of b_oob_m2 is TRUE */
	...
	uint8 *authorizedMacs_buf, int authorizedMacs_len)
{
	...
	/* Serial Number */
	tlv_serialize(WPS_ID_SERIAL_NUM, bufObj, info->serialNumber,
		STR_ATTR_LEN(info->serialNumber, SIZE_32_BYTES));
...
add_wildcard_mac:
				 ...
				 */
				/* caller MUST call tlv_delete to free ssrmsg->authorizedMacs */
				subtlv_dserialize(&ssrmsg->authorizedMacs,
					WPS_WFA_SUBID_AUTHORIZED_MACS,
					wlidcardMac_bufObj, 0, 1);
			}
...
/* The pub_key_hash is peer pub_key_hash */
uint32
wpsap_start_enrollment_devpwid(void *mc_dev, char *ap_pin, uint8 *pub_key_hash,
	uint16 devicepwid)
{
	WPSAPI_T *g_mc = (WPSAPI_T *)mc_dev;
	DevInfo *dev_info = g_mc->dev_info;
	...
	uint16 devicepwid)
{
	...
	if (dev_info->devPwdId == WPS_DEVICEPWDID_NFC_CHO && strcmp(sta_pin, "NFC_CHO") == 0)
		dev_info->b_oob_m2 = TRUE;
	else
		dev_info->b_oob_m2 = FALSE;
