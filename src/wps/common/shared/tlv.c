 ...
 *
 * $Id: tlv.c 475022 2014-05-02 23:21:49Z $
 */
...
tlv_oobDevPwdParse(CTlvOobDevPwd *tlv, BufferObj *theBuf)
{
	 ...
	 */
	/* Note: one byte of password data is not correct,
	 * in the NFC-connection Handover case the Device password shall have zero length
	 */
...
void
tlv_secDeviceTypeWrite(CTlvSecDeviceType *tlv, BufferObj *theBuf)
{
	uint8 temp[sizeof(uint32)];

	cplxtlv_writeHdr(&tlv->base, WPS_ID_SEC_DEV_TYPE_LIST, SIZE_8_BYTES, theBuf);
...
subtlv_getid(uint8 theId)
{
	...
	case WPS_WFA_SUBID_AUTHORIZED_MACS:
		return TLV_UINT8_PTR;
	/* uint16 */
	case WPS_WFA_SUBID_REG_CFG_METHODS:
		return TLV_UINT16;
	}
