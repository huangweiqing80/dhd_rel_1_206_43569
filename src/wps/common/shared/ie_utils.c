...
ie_utils_build_proberesp_IE(void *params, uint8 *buf, int *buflen)
{
	...
	CTlvSecDeviceType tlvSecDeviceType;
#endif	
	CTlvVendorExt vendorExt;
	BufferObj *bufObj = NULL, *vendorExt_bufObj = NULL;
	...
	tlv_primDeviceTypeWrite(&tlvPrimDeviceType, bufObj);
		...
		tlvSecDeviceType.categoryId    = preboresp->secDeviceCategory;
		tlvSecDeviceType.subCategoryId = preboresp->secDeviceSubCategory;
		tlvSecDeviceType.oui = preboresp->secDeviceOui;
		tlv_secDeviceTypeWrite(&tlvSecDeviceType, bufObj);
#endif
