 ...
 *
 * $Id: reg_sm.c 470127 2014-04-14 04:14:51Z $
 */
...
reg_sm_handleMessage(RegSM *r, BufferObj *msg, BufferObj *outmsg)
{
...
#ifdef WPS_NFC_DEVICE
			r->err_code = err;
			/* Registrar send M2D when OOB Public Key Hash mismatch */
			if (err == RPROT_ERR_DEV_PW_ID_MISMATCH ||
			    err == RPROT_ERR_PUB_KEY_HASH_MISMATCH) {
				b_ignore_m2d = false;
				b_oob_att_matched = false;

				/* Based on the discussion with Dave we set WPS_ERROR_PKH_MISMATCH
				 * for NACK packet in the case of PKH mismatch and set No Error
				 * for the case of Device Password ID mismatch
				 */
				if (err == RPROT_ERR_PUB_KEY_HASH_MISMATCH)
					regInfo->registrar->configError = WPS_ERROR_PKH_MISMATCH;
				else
					regInfo->registrar->configError = WPS_ERROR_NO_ERROR;
			}
	...
	case M2:
		if (regInfo->dev_info->b_oob_m2 == TRUE) {
			if (WPS_ID_MESSAGE_DONE != msgType) {
				...
				if (configError == WPS_ERROR_PKH_MISMATCH)
					r->err_code = RPROT_ERR_PUB_KEY_HASH_MISMATCH;
#endif
...
pinfail:
	...
	if (r->reg_info->e_lastMsgSent == M4 || r->reg_info->e_lastMsgSent == M6) {
...
reg_proto_process_m1(RegData *regInfo, BufferObj *msg)
{
...
#ifdef WPS_NFC_DEVICE
	if (IS_OOB_DEVPWID(regInfo->registrar->devPwdId)) {
		/* Device Password ID check */
		if (regInfo->registrar->b_oob_m2 == FALSE &&
		...
		if (regInfo->registrar->b_oob_m2 == TRUE &&
		...
		WPS_HexDumpAscii(TUDUMP_NFC, "Your M1 Public Key Hash Is", DHKey, SIZE_20_BYTES);
#endif
	...
	wps_setStaDevName(enrollee->deviceName);
	...
	default:
		if (!b_oob_devpw) {
			...
			/* WPS+NFC-PF1:
			 * We should not check the Device Password ID for OOBand, because in
			 * proxy mode I don't know what the device password ID will be
			 */
			/* return WPS_ERR_NOT_IMPLEMENTED; */
		}
	}

	TUTRACE((TUTRACE_ERR, "M1: Device Password ID 0x%x [OOB_DEVPW=%s]\n",
		m.devPwdId.m_data, b_oob_devpw ? "TRUE" : "FALSE"));
...
reg_proto_create_m2(RegData *regInfo, BufferObj *msg)
{
	...
	bool b_zlength = registrar->b_zlength;
#endif /* WFA_WPS_20_TESTBED */
...
#ifdef WFA_WPS_20_NFC_TESTBED_DEBUG
{
	...
	WPS_HexDumpAscii(TUDUMP_NFC, "My M2 Public Key Hash Is", DHKey, SIZE_20_BYTES);
}
#endif
