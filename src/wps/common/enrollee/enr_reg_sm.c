 ...
 *
 * $Id: enr_reg_sm.c 470764 2014-04-16 08:40:23Z $
 */
...
#define STR_ATTR_LEN(str, max) \
	((uint16)((b_zpadding && strlen(str) < max) ? strlen(str) + 1 : \
	(b_zlength) ? 0 : strlen(str)))
#else /* !WFA_WPS_20_TESTBED */
#define STR_ATTR_LEN(str, max)	((uint16)(strlen(str)))
#endif /* WFA_WPS_20_TESTBED */

#define IS_OOB_DEVPWID(dpid)	(((dpid) == WPS_DEVICEPWDID_NFC_CHO) || ((dpid) & 0xFFF0))
...
enr_sm_handleMessage(EnrSM *e, BufferObj *msg, BufferObj *outmsg)
{
	...
	case M1:
		...
		if (WPS_ID_MESSAGE_M2D == msgType) {
			err = reg_proto_process_m2d(e->reg_info, msg, regNonce, &configError);
			if (WPS_SUCCESS != err) {
			...
			if (configError == WPS_ERROR_PKH_MISMATCH)
				e->err_code = RPROT_ERR_PUB_KEY_HASH_MISMATCH;
#endif
		...
		/* message wasn't M2D, do processing for M2 */
			...
			if (WPS_SUCCESS != err) {
				...
				else if (err == RPROT_ERR_PUB_KEY_HASH_MISMATCH) {
					TUTRACE((TUTRACE_ERR, "ENRSM: public key hash mismatch "
						"error: SendNack! %x\n", err));
					retVal = state_machine_send_nack(e->reg_info,
						WPS_ERROR_PKH_MISMATCH, outmsg, regNonce);
					e->err_code = err;
					goto out;
				}
				else if (err == RPROT_ERR_DEV_PW_ID_MISMATCH) {
					TUTRACE((TUTRACE_ERR, "ENRSM: Device Password ID mismatch "
						"error: SendNack! %x\n", err));
					retVal = state_machine_send_nack(e->reg_info,
						WPS_ERROR_ROGUE_SUSPECTED, outmsg, regNonce);
					e->err_code = err;
			...
			if (encrSettings) {
				...
				if (err != WPS_SUCCESS) {
					TUTRACE((TUTRACE_ERR, "ENRSM: reg_proto_create_done "
						"generated an error: %d\n", err));
					retVal = err;
				}
				else {
					retVal = WPS_SEND_MSG_CONT;
				...
				if ((!e->reg_info->enrollee->b_ap) ||
					...
					/* case 1: We are not an AP and run an erollee process
					 *         (enr_sm)
...
reg_proto_create_m1(RegData *regInfo, BufferObj *msg)
{
	...
	bool b_zlength = enrollee->b_zlength;
#endif /* WFA_WPS_20_TESTBED */
...
#ifdef WFA_WPS_20_NFC_TESTBED_DEBUG
{
	...
	WPS_HexDumpAscii(TUDUMP_NFC, "My M1 Public Key Hash Is", DHKey, SIZE_20_BYTES);
}
#endif
...
#ifdef WPS_NFC_DEVICE
	if (IS_OOB_DEVPWID(regInfo->enrollee->devPwdId)) {
		/* Device Password ID check */
		if (regInfo->enrollee->b_oob_m2 == FALSE &&
		    regInfo->enrollee->devPwdId != m.devPwdId.m_data) {
			TUTRACE((TUTRACE_ERR, "Device Password ID not matched, R:0x%x E:0x%x\n",
				regInfo->enrollee->devPwdId, m.devPwdId.m_data));
			err = RPROT_ERR_DEV_PW_ID_MISMATCH;
			goto error;
		}

		if (regInfo->enrollee->b_oob_m2 == TRUE &&
		    m.devPwdId.m_data != WPS_DEVICEPWDID_NFC_CHO) {
			TUTRACE((TUTRACE_ERR, "Device Password ID should be 0x%x but got 0x%x\n",
				WPS_DEVICEPWDID_NFC_CHO, m.devPwdId.m_data));
			err = RPROT_ERR_DEV_PW_ID_MISMATCH;
			goto error;
		}
		...
		/* Public Key Hash check */
		if (regInfo->enrollee->b_oob_m2 == TRUE) {
			if (SHA256(regInfo->pkr, SIZE_PUB_KEY, DHKey) == NULL) {
				TUTRACE((TUTRACE_ERR, "Public key hash: SHA256 "
					"calculation failed\n"));
...
#ifdef WFA_WPS_20_NFC_TESTBED_DEBUG
			WPS_HexDumpAscii(TUDUMP_NFC, "I Say Your Public Key Hash Is",
				regInfo->dev_info->pub_key_hash, SIZE_20_BYTES);
			WPS_HexDumpAscii(TUDUMP_NFC, "Your M2 Public Key Hash Is",
				DHKey, SIZE_20_BYTES);
#endif

			if (memcmp(regInfo->dev_info->pub_key_hash, DHKey, SIZE_20_BYTES) != 0) {
				TUTRACE((TUTRACE_ERR, "Public key hash doesn't matched\n"));
				err = RPROT_ERR_PUB_KEY_HASH_MISMATCH;
				goto error;
			}
		}
	}
...
#ifdef WFA_WPS_20_NFC_TESTBED_DEBUG
{
	uint8 priv_key[SIZE_PUB_KEY];

	if (reg_proto_BN_bn2bin(regInfo->enrollee->DHSecret->priv_key, priv_key) == SIZE_PUB_KEY)
		WPS_HexDumpAscii(TUDUMP_KEY, "DH Private Key", priv_key, SIZE_PUB_KEY);
	WPS_HexDumpAscii(TUDUMP_KEY, "DH own Public Key", regInfo->pke, SIZE_PUB_KEY);
	WPS_HexDumpAscii(TUDUMP_KEY, "DH peer Public Key", regInfo->pkr, SIZE_PUB_KEY);
	WPS_HexDumpAscii(TUDUMP_KEY, "DH shared key", secret, SIZE_PUB_KEY);
	WPS_HexDumpAscii(TUDUMP_KEY, "DHKey", DHKey, SIZE_256_BITS);
	WPS_HexDumpAscii(TUDUMP_KEY, "KDK", kdk, SIZE_256_BITS);
	WPS_HexDumpAscii(TUDUMP_KEY, "AuthKey", regInfo->authKey->pBase, SIZE_256_BITS);
	WPS_HexDumpAscii(TUDUMP_KEY, "KeyWrapKey", regInfo->keyWrapKey->pBase, SIZE_128_BITS);
	WPS_HexDumpAscii(TUDUMP_KEY, "EMSK", regInfo->emsk->pBase, SIZE_256_BITS);
}
#endif /* WFA_WPS_20_NFC_TESTBED_DEBUG */
...
static uint32
reg_proto_process_m2d(RegData *regInfo, BufferObj *msg, uint8 *regNonce, uint16 *configError)
{
	WpsM2D m;
	...
	/* Save the peer report error code */
	if (configError)
...
#ifdef WFA_WPS_20_NFC_TESTBED_DEBUG
	WPS_HexDumpAscii(TUDUMP_KEY, "Enrollee: PSK1 Hash", regInfo->psk1, SIZE_128_BITS);
	WPS_HexDumpAscii(TUDUMP_KEY, "Enrollee: PSK2 Hash", regInfo->psk2, SIZE_128_BITS);
#endif
