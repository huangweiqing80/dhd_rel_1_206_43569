 ...
 *
 * $Id: enr_api.c 475022 2014-05-02 23:21:49Z $
 */
...
uint32
wpssta_start_enrollment_devpwid(char *dev_pin, uint8 *pub_key_hash, uint16 devicepwid,
	unsigned long time)
{
	DevInfo *dev_info = g_mc->dev_info;
	...
	if (dev_info->devPwdId == WPS_DEVICEPWDID_NFC_CHO && strcmp(dev_pin, "NFC_CHO") == 0)
		dev_info->b_oob_m2 = TRUE;
	else
		dev_info->b_oob_m2 = FALSE;
	...
	unsigned long time)
{
	...
	if (dev_info->devPwdId == WPS_DEVICEPWDID_NFC_CHO && strcmp(ap_pin, "NFC_CHO") == 0)
		dev_info->b_oob_m2 = TRUE;
	else
		dev_info->b_oob_m2 = FALSE;
...
wpssta_get_err_code()
{
	if (!g_mc || !g_mc->dev_info || !g_mc->mp_enrSM)
		return 0;

	return g_mc->mp_enrSM->err_code;
}
	...
	uint16 assocState, uint16 configError, uint16 passwId)
{
	...
	if (data8 >= WPS_VERSION2) {
		/* Manufacturer */
		tlv_serialize(WPS_ID_MANUFACTURER, bufObj, info->manufacturer,
			STR_ATTR_LEN(info->manufacturer, SIZE_64_BYTES));

		/* Model Name */
		tlv_serialize(WPS_ID_MODEL_NAME, bufObj, info->modelName,
			STR_ATTR_LEN(info->modelName, SIZE_32_BYTES));

		/* Model Number */
		tlv_serialize(WPS_ID_MODEL_NUMBER, bufObj, info->modelNumber,
			STR_ATTR_LEN(info->modelNumber, SIZE_32_BYTES));

		/* Device Name */
		tlv_serialize(WPS_ID_DEVICE_NAME, bufObj, info->deviceName,
			STR_ATTR_LEN(info->deviceName, SIZE_32_BYTES));
		...
		if (info->nattr_len && info->nattr_tlv)
			buffobj_Append(bufObj, info->nattr_len, (uint8*)info->nattr_tlv);
#endif /* WFA_WPS_20_TESTBED */
	}
...
/* A PIN select reg AP can be an unconfigure AP */
int
wps_get_select_aplist(wps_ap_list_info_t *list_in, wps_ap_list_info_t *list_out)
{
	wps_ap_list_info_t *ap_in = &list_in[0];
	wps_ap_list_info_t *ap_out = &list_out[0];
	int i = 0, wps_apcount = 0;
...
wps_get_pin_aplist(wps_ap_list_info_t *list_in, wps_ap_list_info_t *list_out)
{
	...
	while (ap_in->used == TRUE && i < WPS_MAX_AP_SCAN_LIST_LEN) {
		if (true == wps_get_select_reg(ap_in) &&
		    false == is_SelectReg_PBC(ap_in->ie_buf, ap_in->ie_buflen)) {
			if (is_ConfiguredState(ap_in->ie_buf, ap_in->ie_buflen))
				ap_in->scstate = WPS_SCSTATE_CONFIGURED;
			else
				ap_in->scstate = WPS_SCSTATE_UNCONFIGURED;
			/* WSC 2.0 */
			is_wpsVersion2(ap_in->ie_buf, ap_in->ie_buflen,
				&ap_in->version2, ap_in->authorizedMACs);
