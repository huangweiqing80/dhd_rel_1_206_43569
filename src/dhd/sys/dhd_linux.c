 ...
 *
 * $Id: dhd_linux.c 477973 2014-05-15 05:33:12Z $
 */
 ...
 * managing. dhd_forward() may invoke dhd_start_xmit(). dhd_start_xmit() also
 * acquires the spinlock for the dhd unit to which the packet is destined to.
 *
 * As dhd_start_xmit() is also invoked by dhd_forward, a trylock is performed
 * when invoked on the same CPU core. If dhd_start_xmit() was invoked from
 * dhd_forward(), i.e. a nested lock acquisition request, then trylock will
 * fail, and dhd_start_xmit() will proceed without blocking on the nested
 * request. If dhd_start_xmit() was not invoked from dhd_forward() (i.e. say
 * a ping from the network stack), then the trylock will do a blocking call.
 *
...
typedef struct dhd_radio_unit {
	...
	int         must_unlock; /* if non zero, then must ignore tryunlock */
#endif /* CONFIG_SMP */
} dhd_radio_unit_t;
...
typedef struct dhd_radio {
	dhd_radio_unit_t unit[FWDER_MAX_RADIO];
	unsigned long irq_flags; /* UniProcessor irq save/restore flags */
	unsigned long irq_try_flags; /* UniProcessor irq save/restore flags */
} dhd_radio_t;
...
static void dhd_perim_radio_reg(int unit, dhd_pub_t * dhdp);

/** Perimeter Lock/Unlock a radio, with a try option */
static inline void dhd_perim_lock_try(int unit, const bool try_flag);
static inline void dhd_perim_unlock_try(int unit, const bool try_flag);

/** Perimeter Lock/Unlock all units hosted on the CPU core. */
static inline void dhd_perim_lock_all(void);
static inline void dhd_perim_unlock_all(void);
...
dhd_perim_radio_init(void)
{
	...
	for (unit = 0; unit < DHD_MAX_RADIO; unit++) {
		dhd_radio_g.unit[unit].dhdp = (dhd_pub_t *)NULL;
#if defined(CONFIG_SMP)
		spin_lock_init(&dhd_radio_g.unit[unit].perim_lock);
		dhd_radio_g.unit[unit].processor_id = unit % NR_CPUS;
		dhd_radio_g.unit[unit].must_unlock = 0; /* spin_trylock_bh state */
#endif /* CONFIG_SMP */
	}
	dhd_radio_g.irq_flags = 0;
	dhd_radio_g.irq_try_flags = 0;
}

static void
dhd_perim_radio_reg(int unit, dhd_pub_t * dhdp)
{
	dhd_radio_g.unit[unit].dhdp = dhdp;
}
...
static inline void
dhd_perim_invoke_all(dhd_perim_bus_cb_t bus_cb)
{
	int unit;
#if defined(CONFIG_SMP)
	const int processor_id = smp_processor_id();
	for (unit = 0; unit < DHD_MAX_RADIO; unit++) {
		if ((dhd_radio_g.unit[unit].dhdp != NULL) & /* bitwise */
		       (dhd_radio_g.unit[unit].processor_id == processor_id)) {
			bus_cb(dhd_radio_g.unit[unit].dhdp->bus);
		}
	}
#else  /* ! CONFIG_SMP */
	for (unit = 0; unit < DHD_MAX_RADIO; unit++) {
		if (dhd_radio_g.unit[unit].dhdp) {
			bus_cb(dhd_radio_g.unit[unit].dhdp->bus);
		}
	}
#endif /* ! CONFIG_SMP */
...
static inline void
dhd_perim_lock_try(int unit, const bool try_flag)
{
#if defined(CONFIG_SMP)
	if (try_flag &&
	        (dhd_radio_g.unit[unit].processor_id == smp_processor_id())) {
		dhd_radio_g.unit[unit].must_unlock =
		    spin_trylock_bh(&dhd_radio_g.unit[unit].perim_lock);
	} else {
		spin_lock_bh(&dhd_radio_g.unit[unit].perim_lock);
		dhd_radio_g.unit[unit].must_unlock = TRUE;
	}
#else  /* ! CONFIG_SMP */
	if (try_flag)
		local_irq_save(&dhd_radio_g.irq_try_flags);
	else
...
{
	dhd_perim_lock_try(DHD_FWDER_UNIT(dhdp->info), FALSE);
}

static inline void
dhd_perim_unlock_try(int unit, const bool try_flag)
{
#if defined(CONFIG_SMP)
	if (!try_flag) {
		spin_unlock_bh(&dhd_radio_g.unit[unit].perim_lock);
		return;
	}

	if (dhd_radio_g.unit[unit].must_unlock) {
		dhd_radio_g.unit[unit].must_unlock = 0;
		spin_unlock_bh(&dhd_radio_g.unit[unit].perim_lock);
	}
#else  /* ! CONFIG_SMP */
	if (try_flag)
		local_irq_restore(&dhd_radio_g.irq_try_flags);
	else
...
{
	dhd_perim_unlock_try(DHD_FWDER_UNIT(dhdp->info), FALSE);
}

static inline void
dhd_perim_lock_all(void)
{
#if defined(CONFIG_SMP)
	int unit;
	const int processor_id = smp_processor_id();
	for (unit = 0; unit < DHD_MAX_RADIO; unit++) {
		if (dhd_radio_g.unit[unit].processor_id == processor_id) {
			spin_lock_bh(&dhd_radio_g.unit[unit].perim_lock); /* non try */
		}
	}
#else
	local_irq_save(&dhd_radio_g.irq_flags);
...
dhd_perim_unlock_all(void)
{
	...
	for (unit = 0; unit < DHD_MAX_RADIO; unit++) {
		if (dhd_radio_g.unit[unit].processor_id == processor_id) {
			spin_unlock_bh(&dhd_radio_g.unit[unit].perim_lock); /* non try */
		}
	}
#else
	local_irq_restore(&dhd_radio_g.irq_flags);
...
#define DHD_PERIM_LOCK_TRY(unit, flag)      dhd_perim_lock_try((unit), (flag))
#define DHD_PERIM_UNLOCK_TRY(unit, flag)    dhd_perim_unlock_try((unit), (flag))
#define DHD_PERIM_LOCK_ALL()                dhd_perim_lock_all()
#define DHD_PERIM_UNLOCK_ALL()              dhd_perim_unlock_all()
...
#ifdef PCIE_FULL_DONGLE
extern void dhd_bus_set_suspend_resume(dhd_pub_t *dhdp, bool state);
#ifdef CUSTOMER_HW4
...
#ifdef PCIE_FULL_DONGLE
#ifdef CUSTOMER_HW4
		dhdp = dhd_get_gobal_dhdp();
#endif /* CUSTOMER_HW4 */
		dhd_bus_set_suspend_resume(dhdp, suspend);
#endif /* PCIE_FULL_DONGLE */
		break;
...
dhd_del_sta(void *pub, int ifidx, void *ea)
{
	...
	if (ifp->parp_enable) {
		...
		dhd_l2_filter_arp_table_update(pub, ifidx, FALSE, ea, FALSE);
	}
#endif /* DHD_L2_FILTER */
	return;
}
                ...
                uint16 flowid)
{
	...
	if ((sta = dhd_find_sta(dhdp, ifidx, ea)) == DHD_STA_NULL)
		return;
 ...
 *  proxy arp feature
 */
int dhd_sendup(dhd_pub_t *dhdp, void *p)
{
	struct sk_buff *skb;
	uchar reorder_info_buf[WLHOST_REORDERDATA_TOTLEN];
	uint reorder_info_len;
...
dhd_sendpkt(dhd_pub_t *dhdp, int ifidx, void *pktbuf)
{
...
#ifdef DHD_L2_FILTER
	if (dhdp->grat_arp && DHD_IF_ROLE_AP(dhdp, ifidx)) {
	    if (dhd_process_gratuitous_arp(dhdp, pktbuf) == BCME_OK) {
			PKTCFREE(dhdp->osh, pktbuf, TRUE);
			return BCME_ERROR;
	    }
	}
	...
	if (ifp->parp_enable && DHD_IF_ROLE_AP(dhdp, ifidx)) {
		ret = dhd_l2fltr_pkt_handle(dhdp, ifidx, pktbuf, TRUE);
...
#ifdef BCM_GMAC3
	DHD_PKTTAG_SET_DATAOFF((dhd_pkttag_fr_t *)PKTTAG(pktbuf), 0);
#endif /* BCM_GMAC3 */
#endif /* BCM_ROUTER_DHD */
	 ...
	 */
	ret = dhd_flowid_update(dhdp, ifidx, WME_PRIO2AC(PKTPRIO(pktbuf)), pktbuf);
	if (ret != BCME_OK) {
	...
	for (cnt = 0; cnt < skb_cnt; cnt++) { /* Process the array of packets */
		...
		if (ETHER_ISUCAST(data)) { /* lookup station in forwarder's WOFA */
			...
			{	/* Locate the destination dhd_sta using WOFA */
				...
				wofa = fwder_lookup(fwder, (uint16 *)data, FWDER_GMAC_PORT);
...
#error "PROP_TXSTATUS and PCIE_FULL_DONGLE based BCM_ROUTER_DHD not compatible"
			...
			if (no_bypass) { /* use non-bypass path */
				skb->dev = ifp->net;
				goto no_bypass_fixup_start_xmit;
			}
...
no_bypass_fixup_start_xmit:
				fwder_fixup(fwder, skb);
			...
			PKTSETFWDERBUF(dhdp->osh, skb); /* used to reduce cache ops */

			/* Save the flowid and the dataoff in the skb's pkttag */
			DHD_PKTTAG_SET_FLOWID((dhd_pkttag_fr_t *)PKTTAG(skb), flowid);
			DHD_PKTTAG_SET_DATAOFF((dhd_pkttag_fr_t *)PKTTAG(skb), dataoff);

			{
				int ret = dhd_bus_txqueue_enqueue(dhdp->bus, skb, flowid);
			...
			fwder_flood(fwder, skb, fwder->osh, FALSE);
		}

	} /* for cnt */

	/* Flush all pending tx queued packets in bus(s) managed on this CPU core */
	dhd_perim_invoke_all(dhd_bus_txqueue_flush);

	DHD_PERIM_UNLOCK_ALL();
...
{
#ifdef DHD_L2_FILTER
	return dhdp->block_ping ? FALSE : TRUE;
#else
	return TRUE;
#endif /* DHD_L2_FILTER */
}
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (dhdp->grat_arp && DHD_IF_ROLE_STA(dhdp, ifidx)) {
		    if (dhd_process_gratuitous_arp(dhdp, pktbuf) == BCME_OK) {
				PKTCFREE(dhdp->osh, pktbuf, FALSE);
				continue;
		    }
		...
		if (ifp->parp_enable && DHD_IF_ROLE_AP(dhdp, ifidx)) {
			int ret = dhd_l2fltr_pkt_handle(dhdp, ifidx, pktbuf, FALSE);
		...
		if ((ifp->fwdh) && ((ntoh16(eh->ether_type) != ETHER_TYPE_BRCM))) {
			...
			if (ETHER_ISUCAST(da)) {
				...
				sta = (dhd_sta_t *)wofa; /* MacDA lookup gives dest station */
				...
				if (dst_ifp != ifp) { /* Inter interface bridging */
					...
					/* No need to try perim lock, if dst_ifp on different radio
					 * as all radios managed by WOFA would be on same core.
					 */
					dhd_sendpkt(&dst_ifp->info->pub, dst_ifp->idx, pktbuf);
					continue;
				...
				if (DHD_IF_ROLE_AP(dhdp, ifidx) && (!ifp->ap_isolate)) {
					void *npkt = PKTDUP(dhdp->osh, pktbuf);
					dhd_sendpkt(dhdp, ifidx, npkt);
				}
			}

fwder_xmit:
			if (fwder_transmit(ifp->fwdh, skb, 1, skb->dev) != FWDER_FAILURE) {
				continue; /* next pkt */
			}
...
int dhd_ioctl_process(dhd_pub_t *pub, int ifidx, dhd_ioctl_t *ioc, void *data_buf)
{
	...
	if (pub->busstate != DHD_BUS_DATA) {
		...
		bcmerror = BCME_DONGLE_DOWN;
		goto done;
#endif /* !defined (OEM_ANDROID) && !defined(BCM_ROUTER_DHD) */
	}
...
#ifdef DHD_L2_FILTER
	ifp->phnd_arp_table = dhd_init_l2_arp_table(dhdpub, ifidx);
...
#endif /* DHD_WMF */
#ifdef DHD_L2_FILTER
		dhd_l2_filter_arp_table_update(dhdpub, ifidx, TRUE, NULL, FALSE);
		dhd_deinit_l2_arp_table(dhdpub, ifp->phnd_arp_table);
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	DHD_FWDER_UNIT(dhd) = fwder_assign(FWDER_DNG_MODE, pcie_probe_unit);
	if (DHD_FWDER_UNIT(dhd) == FWDER_FAILURE) {
		DHD_ERROR(("%s: fwder_assign<%d> failed!\n",
		           __FUNCTION__, pcie_probe_unit));
		goto fail;
	}
	pcie_probe_unit++;

	dhd->pub.fwder_unit = DHD_FWDER_UNIT(dhd);
	...
	if (CTF_ENAB(dhd->cih) && (osl_ctfpool_init(dhd->pub.osh,
		RXBUFPOOLSZ, RXBUFSZ + BCMEXTRAHDROOM) < 0)) {
...
#if defined(OEM_ANDROID) && defined(BCMLXSDMMC) && (LINUX_VERSION_CODE >= \
...
#endif /* DHD_WMF */
#ifdef DHD_L2_FILTER
			dhd_l2_filter_arp_table_update(&dhd->pub, 0, TRUE, NULL, FALSE);
			dhd_deinit_l2_arp_table(&dhd->pub, ifp->phnd_arp_table);
			ifp->phnd_arp_table = NULL;
#endif /* DHD_L2_FILTER */
...
int dhd_set_parp_status(dhd_pub_t *dhdp, uint32 idx, int val)
{
	...
	if (val == FALSE) {
		dhd_l2_filter_arp_table_update(dhdp, idx, TRUE, NULL, FALSE);
	}
	return BCME_OK;
}
