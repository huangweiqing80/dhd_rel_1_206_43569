    ...
    const uint8 *data, int data_len, const uint8 *key, uint8 * mic)
{
    ...
    aes_cmac_calc(micdata, len, key, BIP_KEY_SIZE, mic, BIP_MIC_SIZE);
	free(micdata);
}
	                 ...
	                 uchar* tk, uint tk_sz, uchar* mick, uint   mick_sz)
{
	...
	if (opts->debug) {
		...
		fprintf(stderr, "phase1 key: ");
		p = (uchar *)p1_key;
		for (i = 0; i < TKHASH_P1_KEY_SIZE; ++i) {
			fprintf(stderr, "%02x ", p[i]);
		}
		fprintf(stderr, "\n");

		fprintf(stderr, "phase2 key: ");
		p = (uchar *)rc4_key;
		for (i = 0; i < TKHASH_P2_KEY_SIZE; ++i) {
			fprintf(stderr, "%02x ", p[i]);
		}
		fprintf(stderr, "\n");
	}
	...
	DBG("TKIP MIC: l.r with DA.SA %08x.%08x -> %008x.%08x",
		mic_l, mic_r, l, r);
	mic_l = l; mic_r = r;

	tkip_mic(mic_l, mic_r, 4, (uint8*)&qos4, &l, &r);
	DBG("TKIP MIC: l.r with qos %08x.%08x -> %008x.%08x",
		mic_l, mic_r, l, r);
	mic_l = l; mic_r = r;
	...
	if (opts->debug) {
		fprintf(stderr, "begin data with eom: \n");
		output(2, data, pad_data_len);
		fprintf(stderr, "end data with eom: \n");
	}

	tkip_mic(mic_l, mic_r, pad_data_len, data, &l, &r);
	DBG("TKIP MIC: l.r final %08x.%08x -> %008x.%08x",
		mic_l, mic_r, l, r);
...
uchar* decode(const opts_t* opts, const uchar* in, uint isz, uint *osz)
{
	...
	*osz=0;
	hex2bin((char*)in, isz, out, out_lim, osz);
	DBG("in=%p, isz=%d, out=%p, out_lim=%d, osz=%d", in, isz, out, out_lim, *osz);
	do {
		uint ftype, fsubtype;
		bool is_wds;
