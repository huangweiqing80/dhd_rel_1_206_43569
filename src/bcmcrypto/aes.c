 ...
 *
 * $Id: aes.c 453301 2014-02-04 19:49:09Z $
 */
...
void
aes_encrypt(const size_t kl, const uint8 *K, const uint8 *ptxt, uint8 *ctxt)
{
...
void
aes_decrypt(const size_t kl, const uint8 *K, const uint8 *ctxt, uint8 *ptxt)
{
	uint32 rk[4 * (AES_MAXROUNDS + 1)];
...
int
aes_ccm_encrypt(unsigned int *rk,
                const size_t key_len,
                const uint8 *nonce,
                const size_t aad_len,
                const uint8 *aad,
...
int
aes_ccm_decrypt(unsigned int *rk,
                const size_t key_len,
                const uint8 *nonce,
                const size_t aad_len,
                const uint8 *aad,
...
int
aes_ccmp_encrypt(unsigned int *rk,
                 const size_t key_len,
                 const size_t data_len,
                 uint8 *p,
                 bool legacy,
...
int
aes_ccmp_decrypt(unsigned int *rk,
                 const size_t key_len,
                 const size_t data_len,
                 uint8 *p,
                 bool legacy,
...
void
aes_ccmp_cal_params(const struct dot11_header *h, bool legacy,
                    uint8 nonce_1st_byte, uint8 *nonce, uint8 *aad, uint *la, uint *lh)
{
	...
	if (qos) {
		qc = ltoh16(*((const uint16 *)((const uchar *)h +
	...
	*la = AES_CCMP_AAD_MIN_LEN + addlen;
	/* pointer to IV */
	iv_data = (const uint8 *)h + DOT11_A3_HDR_LEN + addlen;

	*nonce++ = nonce_1st_byte;

	memcpy(nonce, (const uchar *)&h->a2, ETHER_ADDR_LEN);
	...
	*aad++ = (uint8)((seq >> 8) & 0xff);
	...
	if (wds) {
		memcpy(aad, (const uchar *)&h->a4, ETHER_ADDR_LEN);
		aad += ETHER_ADDR_LEN;
	}
	if (qos) {
		if (!legacy) {
 ...
 */
void
aes_cmac_gen_subkeys(const int nrounds, const uint32 *rk, uint8 *K1, uint8 *K2)
{
	...
	/* L = CIPHK(0b) */
	rijndaelEncrypt(rk, nrounds, Z128, L);
	pres("Key", kl, K);
 ...
 */
void
aes_cmac(const int nrounds, const uint32* rk,
	...
	const size_t mac_len)
{
	...
	*/
	...
	for (i = 1; i < n; i++) {
		...
		aes_block_encrypt(nrounds, rk, tmp, C);
	}
	xor_128bit_block(C, Mn, tmp);
	aes_block_encrypt(nrounds, rk, tmp, C);

	/* Let T = MSBTlen(Cn) */
	memcpy(mac, C, MIN(AES_CMAC_AUTH_LEN, mac_len));
...
aes_cmac_calc(const uint8 *data, const size_t data_length, const uint8 *mic_key,
	const size_t key_len, uint8 *mic, const size_t mic_len)
{
	uint8 K1[AES_BLOCK_SZ], K2[AES_BLOCK_SZ];
	uint32 rk[4 * (AES_MAXROUNDS + 1)];
	int nrounds;

	nrounds = (int)AES_ROUNDS(key_len);
	rijndaelKeySetupEnc(rk, mic_key, (int)AES_KEY_BITLEN(key_len));

	aes_cmac_gen_subkeys(nrounds, rk, K1, K2);
	aes_cmac(nrounds, rk, K1, K2, data_length, data, mic, mic_len);
}
