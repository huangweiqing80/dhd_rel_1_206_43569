 ...
 *
 * $Id: wlu.c 477911 2014-05-15 01:03:52Z $
 */
...
static cmd_func_t wl_rssi, wl_rssi_event, wl_chan_qual_event, wl_gmode;
...
static cmd_func_t wl_var_getint, wl_var_getinthex;
...
static const struct tlv_info_list nan_attr_list[] = {
		{"master", WL_NAN_XTLV_MASTER_PREF},
		{"cluster_id", WL_NAN_XTLV_CLUSTER_ID},
		{"if_addr", WL_NAN_XTLV_IF_ADDR},
		{"mcast_addr", WL_NAN_XTLV_MC_ADDR},
		{"role", WL_NAN_XTLV_ROLE},
		{"bcn_int", WL_NAN_XTLV_BCN_INTERVAL},
		{"dw_len", WL_NAN_XTLV_DW_LEN},
		{NULL, 0}
};
...
static const wl_nan_sub_cmd_t nan_cmd_list[] = {
	...
	WL_NAN_XTLV_IF_ADDR, WL_NAN_FUNC(set_attr)
	},
	...
	IOVT_BUFFER, WL_NAN_FUNC(enable)
	},
...
static int print_nan_ssinfo(uint8 *p, uint16 len)
{
	char buf[20];
	uint8 mapcontrol;
	uint8 proto;
 ...
 */
int wlu_nan_set_vars_cbfn(void *ctx, void **tlv_buf, uint16 type, uint16 len)
{
	int res = BCME_OK;
	...
	UNUSED_PARAMETER(msg);
		switch (type) {
			case WL_NAN_XTLV_ENABLE:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_ENABLE, sizeof(uv8), &uv8);
				printf("> nan: %s\n", uv8?"enabled":"disabled");
				break;
			case WL_NAN_XTLV_MASTER_PREF:
			/* master role and preference  mac has them as two u8's  */
				if ((res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_MASTER_PREF,
					sizeof(uint16), &uv16)) == BCME_OK) {
			...
			/* interface address */
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_IF_ADDR, ETHER_ADDR_LEN, &mac);
				bcm_ether_ntoa(&mac, buf);
				printf("> if_addr:    %s\n", buf);
				break;
			case WL_NAN_XTLV_CLUSTER_ID:
			/* interface address */
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_CLUSTER_ID, ETHER_ADDR_LEN, &mac);
			...
			case WL_NAN_XTLV_ROLE:
				...
				res = bcm_unpack_xtlv_entry(tlv_buf, WL_NAN_XTLV_ROLE, 4, &uv32);
				print_nan_role(uv32);
				break;
			case WL_NAN_XTLV_BCN_INTERVAL:
				/* discovery beacon interval */
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_BCN_INTERVAL, sizeof(uint16), &uv16);
				printf("> bcn interval:%d\n", uv16);
				break;
			case WL_NAN_XTLV_DW_LEN:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_DW_LEN, sizeof(uint16), &uv16);
				printf("> discovery window length:%d\n", uv16);
				break;
			case WL_NAN_XTLV_MAC_INITED:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_MAC_INITED, sizeof(uint8), &uv8);
				printf("> inited:%d\n", uv8);
				break;
			case WL_NAN_XTLV_MAC_ENABLED:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_MAC_ENABLED, sizeof(uint8), &uv8);
				printf("> joined:%d\n", uv8);
				break;
			case WL_NAN_XTLV_MAC_CHANSPEC:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_MAC_CHANSPEC, sizeof(chanspec_t), &chanspec);
			...
			case WL_NAN_XTLV_MAC_AMR:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_MAC_AMR, NAN_MASTER_RANK_LEN, buf);
				prhex("> amr", (uint8 *)buf, NAN_MASTER_RANK_LEN);
				break;
			case WL_NAN_XTLV_MAC_HOPCNT:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_MAC_HOPCNT, sizeof(uint8), &uv8);
				printf("> hop_count:%d\n", uv8);
				break;
			case WL_NAN_XTLV_MAC_AMBTT:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_MAC_AMBTT, sizeof(uint32), &uv32);
				printf("> ambtt:0x%x\n", uv32);
				break;
			case WL_NAN_XTLV_MAC_TXRATE:
			res = bcm_unpack_xtlv_entry(tlv_buf,
				WL_NAN_XTLV_MAC_TXRATE, sizeof(uint32), &uv32);
			printf("> ratespec:0x%x\n", uv32);
			break;
			case WL_NAN_XTLV_INSTANCE_ID:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_INSTANCE_ID, sizeof(wl_nan_instance_id_t),
					&uv8);
				printf("------------------\n");
			...
			case WL_NAN_XTLV_SVC_NAME:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_SVC_NAME, WL_NAN_SVC_HASH_LEN, buf);
				printf("> Service Name:%.*s\n", WL_NAN_SVC_HASH_LEN, buf);
				break;
			case WL_NAN_XTLV_SVC_PARAMS: {
					wl_nan_disc_params_t params;

					res = bcm_unpack_xtlv_entry(tlv_buf,
						WL_NAN_XTLV_SVC_PARAMS,
						sizeof(wl_nan_disc_params_t), &params);
			...
			case WL_NAN_XTLV_MAC_STATUS: {
					wl_nan_status_t nstatus;
					res = bcm_unpack_xtlv_entry(tlv_buf,
						WL_NAN_XTLV_MAC_STATUS, sizeof(wl_nan_status_t),
						&nstatus);
					if (res != BCME_OK)
						break;
					printf("> inited:%d\n", nstatus.inited);
			...
			case WL_NAN_XTLV_NAN_SCANPARAMS: {
					...
					res = bcm_unpack_xtlv_entry(tlv_buf,
						WL_NAN_XTLV_NAN_SCANPARAMS,
						sizeof(nan_scan_params_t),
						&scan_params);
					if (res != BCME_OK)
						break;
					...
					*--chlist = 0;
					printf("scan params: scan_time:%d,"
						" home_time:%d, chcnt:%d, chlist:%s\n",
						dtoh16(scan_params.scan_time),
						dtoh16(scan_params.home_time), chnum, buf);
				}
				break;
			...
			case WL_NAN_XTLV_DEBUGPARAMS: {
				...
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_DEBUGPARAMS,
					sizeof(nan_debug_params_t),
					&params);
			...
			case WL_NAN_XTLV_EVENT_MASK:
				...
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_EVENT_MASK, sizeof(uint32), &uv32);
			...
			case WL_NAN_XTLV_PTBTT_OVERRIDE:
				/*  TODO: add pre_tbtt_override to wlu.c (if needed)  */
				/* res |= bcm_unpack_xtlv_entry(&pxtlv,
					NAN_TLV_PTBTT_OVERRIDE, sizeof(uint16),
					&nmi->pre_tbtt_override);
					if (res != BCME_OK) goto fail;
				*/
				break;
			case WL_NAN_XTLV_PUBLR_ID:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_PUBLR_ID, sizeof(uint8), &uv8);
				printf("------------------\n");
				printf("> Publish ID: %d\n", uv8);
				break;
			case WL_NAN_XTLV_SUBSCR_ID:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_SUBSCR_ID, sizeof(uint8), &uv8);
				printf("------------------\n");
				printf("> Subscriber ID: %d\n", uv8);
				break;
			case WL_NAN_XTLV_MAC_ADDR:
				res = bcm_unpack_xtlv_entry(tlv_buf,
					WL_NAN_XTLV_MAC_ADDR, ETHER_ADDR_LEN, &mac);
				bcm_ether_ntoa(&mac, buf);
				printf("> macaddr:    %s\n", buf);
				break;
			case WL_NAN_XTLV_SVC_INFO:
				ptlv = *tlv_buf;
				size = BCM_XTLV_LEN(ptlv);
				data = malloc(size);
				if (data) {
					bcm_unpack_xtlv_entry(tlv_buf,
						WL_NAN_XTLV_SVC_INFO, size, data);
					res = print_nan_ssinfo(data, size);
					free(data);
				} else
					res = BCME_NOMEM;
				break;
...
wl_nan_subcmd_event_check(void *wl, const wl_nan_sub_cmd_t  *cmd, char **argv)
{
	...
	while (1) {
		...
		if (err == BCME_OK)
			err = bcm_unpack_xtlv_buf(&nan_evtnum,
				nandata, datalen, wlu_nan_set_vars_cbfn);
	}
	free(data);
exit1:
	close(fd);
...
/*  check the response buff  */
	if ((res == BCME_OK) && (iocresp != NULL)) {
		...
		res = bcm_unpack_xtlv_buf(nanioc,
			iocresp->data, iocresp->len, wlu_nan_set_vars_cbfn);
	}
	...
	*/
	...
	/* get: handled by cbfn */
		...
		res = bcm_pack_xtlv_entry(&pxtlv,
			...
			sizeof(nan_debug_params_t), &params);
...
wl_nan_subcmd_scan_params(void *wl, const wl_nan_sub_cmd_t  *cmd, char **argv)
{
	...
	nanioc->version = htod16(WL_NAN_IOCTL_VERSION);
	nanioc->id = cmd->id;
	nanioc->len = BCM_XTLV_HDR_SIZE + sizeof(nan_scan_params_t);
	iocsz = sizeof(wl_nan_ioc_t) + nanioc->len;
	...
	/* get: handled by cbfn */
		...
		res = bcm_pack_xtlv_entry(&pxtlv,
			...
			sizeof(nan_scan_params_t), &scan_params);
...
wl_nan_subcmd_scan(void *wl, const wl_nan_sub_cmd_t  *cmd, char **argv)
{
	...
	res = bcm_pack_xtlv_entry(&pxtlv,
		&buflen, WL_NAN_XTLV_CLUSTER_ID, ETHER_ADDR_LEN, &cid);
	if (res != BCME_OK)
		goto exit;
...
wl_nan_subcmd_enable(void *wl, const wl_nan_sub_cmd_t  *cmd, char **argv)
{
	...
	if (*argv == NULL) { /* get  */
		...
		res = bcm_pack_xtlv_entry(&pxtlv,
			&buflen, WL_NAN_XTLV_ENABLE,  sizeof(uint8), &val);
		if (res != BCME_OK) goto fail;
...
wl_nan_subcmd_set_attr(void *wl, const wl_nan_sub_cmd_t  *cmd, char **argv)
{
	...
	/* got args, do set  */
		uint16 buflen, buflen_at_start;
		uint16 rnd_pref;
		uint8 master_preference = 2;
		uint8 random_factor = 0x5a;
		struct ether_addr mac = ether_null;
		uint32 role = 0;
		uint16 disc_bcn_interval = 102;
		uint16 dw_len = 18;
		uint32 txrate = 6;
		chanspec_t chspec_b = 1;
		...
		case WL_NAN_XTLV_MASTER_PREF:
			...
			res = bcm_pack_xtlv_entry(&pxtlv,
				&buflen, WL_NAN_XTLV_MASTER_PREF, sizeof(uint16), &rnd_pref);
			break;
		case WL_NAN_XTLV_CLUSTER_ID:
			...
			res = bcm_pack_xtlv_entry(&pxtlv,
				&buflen, WL_NAN_XTLV_CLUSTER_ID, ETHER_ADDR_LEN, &mac);
			break;
		case WL_NAN_XTLV_IF_ADDR:
			if (!wl_ether_atoe(argv[0], &mac)) {
				printf("Malformed MAC address parameter\n");
			...
			res = bcm_pack_xtlv_entry(&pxtlv,
				&buflen, WL_NAN_XTLV_IF_ADDR, ETHER_ADDR_LEN, &mac);
			break;
		case WL_NAN_XTLV_ROLE:
			role = htod32(atoi(argv[0]));
			res = bcm_pack_xtlv_entry(&pxtlv,
				&buflen, WL_NAN_XTLV_ROLE,  4, &role);
			break;
		case WL_NAN_XTLV_BCN_INTERVAL:
			disc_bcn_interval = htod16(atoi(argv[0]));
			res = bcm_pack_xtlv_entry(&pxtlv,
				&buflen, WL_NAN_XTLV_BCN_INTERVAL,
				sizeof(uint16), &disc_bcn_interval);
			break;
		case WL_NAN_XTLV_MAC_CHANSPEC:
			...
			if (argv[0]) {
				...
				if (chspec_b != 0) {
					res = bcm_pack_xtlv_entry(&pxtlv,
						&buflen, WL_NAN_XTLV_MAC_CHANSPEC,
						sizeof(chanspec_t), &chspec_b);
				}
			}
			if (argv[1]) { /* rfu  */
				chspec_a = wf_chspec_aton(argv[0]);
				if (chspec_a != 0) {
					res = bcm_pack_xtlv_entry(&pxtlv,
						&buflen, WL_NAN_XTLV_MAC_CHANSPEC,
						sizeof(chanspec_t), &chspec_a);
				}
			}
			break;
		...
		case WL_NAN_XTLV_MAC_TXRATE:
			...
			res = bcm_pack_xtlv_entry(&pxtlv,
				&buflen, WL_NAN_XTLV_MAC_TXRATE,  sizeof(uint32), &txrate);
			break;
		case WL_NAN_XTLV_DW_LEN:
			dw_len = htod16(atoi(argv[0]));
			res = bcm_pack_xtlv_entry(&pxtlv,
				&buflen, WL_NAN_XTLV_DW_LEN,  sizeof(uint16), &dw_len);
				break;
		default:
			res = BCME_BADARG;
			break;
...
wl_nan_subcmd_join(void *wl, const wl_nan_sub_cmd_t *cmd, char **argv)
{
	...
	res = bcm_pack_xtlv_entry(&pxtlv,
		&buflen, WL_NAN_XTLV_START,  1, &start);
	if (res != BCME_OK) goto exit;
...
wl_nan_subcmd_leave(void *wl, const wl_nan_sub_cmd_t *cmd, char **argv)
{
	...
	wl_nan_ioc_t*nanioc = NULL;
	struct ether_addr id = ether_null;
	if (*argv) {
		if (!wl_ether_atoe(*argv, &id)) {
			printf("%s : Malformed MAC address parameter\n", __FUNCTION__);
			ret = BCME_USAGE_ERROR;
			goto exit;
		}
	...
	tbuf = bcm_xtlv_buf_alloc(NULL, tot_len);
	if (tbuf == NULL) {
		ret = BCME_NOMEM;
		goto exit;
	}
	if (memcmp(&id, &ether_null, sizeof(struct ether_addr)))
		bcm_xtlv_put_data(tbuf, WL_NAN_XTLV_CLUSTER_ID, &id, sizeof(struct ether_addr));

	iocsz = sizeof(wl_nan_ioc_t) + bcm_xtlv_buf_len(tbuf);
	nanioc = calloc(1, iocsz);
	if (nanioc == NULL) {
		ret = BCME_NOMEM;
		goto exit;
	}
	nanioc->version = htol16(WL_NAN_IOCTL_VERSION);
	nanioc->id = htol16(cmd->id);
	nanioc->len = htol16(bcm_xtlv_buf_len(tbuf));
	bcopy(bcm_xtlv_head(tbuf), nanioc->data, bcm_xtlv_buf_len(tbuf));
	ret = wlu_var_setbuf(wl, "nan", nanioc, iocsz);
	if (ret < 0) {
		fprintf(stderr, "failed to execute nan leave : %d\n", ret);
		prhex("ioc parambuf:", (void *)nanioc, iocsz);
	}
exit:
	if (tbuf)
...
wl_nan_subcmd_svc(void *wl, const wl_nan_sub_cmd_t *cmd, char **argv, uint32 default_flags)
{
	...
	int int_val = 0;
	wl_nan_disc_params_t params;
	uint tot_len = BCM_XTLV_HDR_SIZE + sizeof(params);
	struct bcm_tlvbuf *tbuf = NULL;
	uint iocsz = 0;
	wl_nan_ioc_t *nanioc = NULL;
	char *param, *val_p, *endptr = NULL;
	uint32 val = 0;
	...
	while ((param = *argv++) != NULL) {
		/* Check that value is provided and that it is numeric */
		if ((val_p = *argv++) == NULL) {
	...
	ret = wlu_var_setbuf(wl, "nan", nanioc, iocsz);
	if (ret < 0) {
		fprintf(stderr, "failed to execute nan service : %d\n", ret);
		prhex("ioc parambuf", (void *)nanioc, iocsz);
	}
exit:
	if (tbuf)
...
{
	int ret = BCME_OK;
	uint tot_len = BCM_XTLV_HDR_SIZE + sizeof(wl_nan_instance_id_t);
	uint iocsz = 0;
	struct bcm_tlvbuf *tbuf = NULL;
	wl_nan_ioc_t *nanioc = NULL;
	int int_val = 0;
	...
	instance_id = (uint8)int_val;
	tbuf = bcm_xtlv_buf_alloc(NULL, tot_len);
	if (tbuf == NULL) {
		ret = BCME_NOMEM;
		goto exit;
	}
	bcm_xtlv_put_data(tbuf, WL_NAN_XTLV_INSTANCE_ID, &instance_id, sizeof(instance_id));
	...
	ret = wlu_var_setbuf(wl, "nan", nanioc, iocsz);
	if (ret < 0) {
		fprintf(stderr, "failed to execute nan cancel_publish : %d\n", ret);
		prhex("ioc parambuf", (void *)nanioc, iocsz);
	}
exit:
	if (tbuf)
