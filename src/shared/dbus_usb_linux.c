 ...
 *
 * $Id: dbus_usb_linux.c 477178 2014-05-12 21:12:41Z $
 */
...
dbus_usbos_intf_send_ctl(void *bus, uint8 *buf, int len)
{
	...
	wake_up_interruptible(&usbos_info->usbos_tx_queue_head);
#else
	if ((ret = USB_SUBMIT_URB(usbos_info->ctl_urb))) {
