...
#ifdef ANDROID_P2P
static void wpas_p2p_canc_remain_on_channel_cb(void *eloop_ctx,
					     void *freq_value)
{
	struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)eloop_ctx;
	unsigned int *freq = (unsigned int *)freq_value;
	wpa_printf(MSG_DEBUG, "P2P: wpas_p2p_canc_remain_on_channel_cb");
	wpas_p2p_cancel_remain_on_channel_cb(wpa_s, *freq);
}
#endif
...
#ifdef ANDROID_P2P
	if (offchannel_pending_action_tx(wpa_s)) {
	/* Make sure that we call the cb again so that listen is
	* continued
	 */
	 if (wpa_s->p2p_long_listen > 0)
	    eloop_register_timeout(0, 100,
		wpas_p2p_canc_remain_on_channel_cb, wpa_s, (void *)&freq);
		return;
	}
#endif
	if (wpa_s->p2p_long_listen > 0) {
